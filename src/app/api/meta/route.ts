...
export async function GET() {
  ...
  // Compute top build per activity
  ...
  // Meta Alerts — builds with highest peak scores (rising)
  ...
  // Emerging builds — mid-tier gaining (score 70-85)
  ...
  // Creator signals — unique creators from top builds
  ...
  const [consensusAlerts, patchImpact, topForecasters] = await Promise.all([
    ...
    import("@/server/services/TrustService").then(m => m.TrustService.getTopForecasters(5))
  ]);
